...
% idx_long = 2;
% idx_cellid = 14;
% cell_total = cell_wx_outdoor(:, [idx_lat, idx_long, idx_cellid]);
% 
% 
...
% 
% 
wx_msr = measurement(:, [2, 18, 17]);

wx_msr = sortrows(wx_msr, [1, 2, 3]);
...
%     cellid = cellid - fix(cellid/2^16)*2^16;
    ...
    idx_msr_start = find(wx_msr(:, 1)==cellid, 1, 'first'); 
    idx_msr_end = find(wx_msr(:, 1)==cellid, 1, 'last');
    ...
    for jj=1:cell_count
        ...
        msr_lat_long = deg2rad(wx_msr(idx_msr_start:idx_msr_end, 2:3));
